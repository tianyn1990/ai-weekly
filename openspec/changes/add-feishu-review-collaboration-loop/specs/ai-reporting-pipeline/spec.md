...
- **WHEN** 审核人在 Feishu 审核卡片中执行通过或打回动作
...
## ADDED Requirements
### Requirement: Weekly pipeline SHALL support feedback-driven revision loop
系统 SHALL 支持审核意见回流修订能力，在保留原有内容基础上执行结构化调整，并在修订后重新进入终稿审核。

#### Scenario: Reviewer requests revision with structured directives
- **WHEN** 审核人提交 `request_revision` 且包含结构化指令（新增/删除候选、主题词/搜索词调整、来源启停或权重调整、排序权重调整）
- **THEN** 系统基于当前待审核版本执行修订
- **AND** 修订结果进入终稿审核阶段

#### Scenario: Reviewer rejects current publication attempt
- **WHEN** 审核人提交 `reject` 动作
- **THEN** 系统终止当前发布尝试并保留审计记录
- **AND** 历史待审核产物保持可追溯，不自动删除

#### Scenario: Reject requires a new run for next publication attempt
- **WHEN** 某 reportDate 已被 `reject`
- **THEN** 系统不得在当前 run 内继续推进发布
- **AND** 必须新建 run 才能再次进入发布流程

#### Scenario: Revision scope updates global configuration
- **WHEN** 审核人提交来源开关、来源权重或排序权重调整
- **THEN** 系统将调整写入全局配置
- **AND** 后续周期 run 读取该配置并生效
