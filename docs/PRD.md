...
- 采集适配器：支持 `rss` 与 `github_search` 混合来源；GitHub 热门开源项目优先走一手 API（非第三方镜像）。
- GitHub 鉴权策略：`GITHUB_TOKEN` 可选但强建议；缺失时允许匿名配额运行并输出限流风险提示。
- 默认来源扩展：新增 InfoQ AI/ML 与 Google AI Blog 等实测稳定来源；不稳定来源默认禁用。
- 黑名单：当前为空，但系统必须支持。
...
- M5.2 并发护栏：全局 LLM 并发默认不超过 2（可配置）。
- M5.2 排序策略：在 `rank` 前执行前置 LLM 批量分类+全量打分，再按 `fusionWeight` 融合规则分（默认 0.65，低置信度回退）。
...
- [ ] LLM 触发全局回退需同时满足：成功率低于 70% 且失败条目数不少于 3；否则仅失败条目回退规则摘要。
- [ ] LLM 调用失败/超时/响应非法时自动回退规则摘要，主流程不中断。
- [ ] 单次 run 发生回退时，仅发送 1 条飞书合并告警（不刷屏）。

### 12.10 M5.2（LLM 辅助排序与可读性增强）验收
- [ ] 全局 LLM 并发上限可配置，默认 2，节点并发不超过全局上限。
- [ ] `llm_classify_score` 采用批量请求（默认 batchSize=10），并支持“批次重试 -> 拆批降级 -> 单条回退”容错链路。
- [ ] 当 `missing_content` 连续失败达到阈值时，系统会触发临时串行重试降载，减少批量失败。
- [ ] 报告产物记录 `llmClassifyScoreMeta` 与融合评分信息（规则分/LLM 分/最终分）并可追溯。
- [ ] 低置信度或异常 LLM 输出会回退规则分，主流程不中断。
- [ ] 报告包含“本期导语”区块，导语失败时回退模板文案。
- [ ] `llm_summarize` 仅负责摘要/导语/导读/翻译，不再改写排序分数。
- [ ] 英文标题可显示“中文标题（原标题）”，翻译失败回退原标题。
- [ ] 摘要与重点若检测到非中文，系统会执行“重试 + 中文修复”链路；修复失败保留英文原文。
- [ ] 结构化产物记录中文质量统计（检测、修复尝试、修复成功、英文保留）。

### 12.11 M5.3（LLM 稳定性与诊断增强）验收
- [ ] `missing_content` 在短窗口达到阈值时会触发自适应降载（优先低并发补偿重试）。
- [ ] 窗口成功率恢复后会自动恢复并发，并在元数据中记录 trigger/recover 统计。
- [ ] `llmSummaryMeta` 包含失败分类、重试统计与自适应降载统计，便于快速归因。
- [ ] 报告新增“分类导读”区块；导读生成失败时回退模板，不阻断主流程。

### 12.12 M5.4（数据源扩展与诊断增强）验收
- [ ] 系统支持 `github_search` 来源类型，并可将结果映射为标准候选条目。
- [ ] 配置 `GITHUB_TOKEN` 时 GitHub 采集使用鉴权请求；未配置时给出限流风险提示。
- [ ] GitHub 限流/鉴权/网络失败仅产出 warning，不阻断日报/周报主流程。
