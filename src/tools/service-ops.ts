...
const DEFAULT_RUNTIME_ROOT = path.join(os.homedir(), ".local", "state", "ai-weekly", "runtime");
...
async function runUp(args: CliArgs, runner: CommandRunner = defaultCommandRunner) {
  ...
  await buildProjectForLaunchd(args, runner);
...
async function ensureLaunchdEnvFile(args: CliArgs): Promise<string> {
  ...
  if (sourceExists) {
    ...
    await prepareLaunchdRuntimeRoot(args);
    await migrateManagedRuntimeState(args);
    const mergedContent = mergeLaunchdEnvContent({
      sourceContent,
      managedEnv: buildManagedLaunchdEnv(sourceEnv, args),
    });
    await fs.writeFile(targetPath, mergedContent, "utf-8");
  } else if (!(await fileExists(targetPath))) {
    await prepareLaunchdRuntimeRoot(args);
    const managedContent = mergeLaunchdEnvContent({
      sourceContent: "# generated by service-ops\n",
      managedEnv: buildManagedLaunchdEnv({}, args),
    });
    await fs.writeFile(targetPath, managedContent, "utf-8");
  }
...
function parseArgs(argv: string[], projectRoot: string, env: NodeJS.ProcessEnv): CliArgs {
  ...
  return {
    ...
    projectRoot,
    envFilePath,
    runtimeRoot: env.AI_WEEKLY_RUNTIME_ROOT ?? DEFAULT_RUNTIME_ROOT,
...
async function prepareLaunchdRuntimeRoot(args: Pick<CliArgs, "runtimeRoot">) {
  // launchd 模式把内部状态迁到低风险目录，避免后台读写 Documents 下的 outputs 被 TCC 拦截。
  await fs.mkdir(args.runtimeRoot, { recursive: true });
}

async function migrateManagedRuntimeState(args: Pick<CliArgs, "projectRoot" | "runtimeRoot">) {
  const mappings = [
    { source: path.join(args.projectRoot, "outputs", "db"), target: path.join(args.runtimeRoot, "db") },
    { source: path.join(args.projectRoot, "outputs", "daemon"), target: path.join(args.runtimeRoot, "daemon") },
    { source: path.join(args.projectRoot, "outputs", "notifications"), target: path.join(args.runtimeRoot, "notifications") },
    { source: path.join(args.projectRoot, "outputs", "review-instructions"), target: path.join(args.runtimeRoot, "review-instructions") },
    { source: path.join(args.projectRoot, "outputs", "runtime-config"), target: path.join(args.runtimeRoot, "runtime-config") },
    { source: path.join(args.projectRoot, "outputs", "watchdog"), target: path.join(args.runtimeRoot, "watchdog") },
  ];
  for (const mapping of mappings) {
    await migrateRuntimePath(mapping.source, mapping.target);
  }
}

async function migrateRuntimePath(sourcePath: string, targetPath: string) {
...
function buildManagedLaunchdEnv(
  sourceEnv: Record<string, string>,
  args: Pick<CliArgs, "runtimeRoot" | "launchdEnvFilePath">,
): Record<string, string> {
  const managed = {
    // CLI 启动阶段会再次调用 env loader，这里固定指回 launchd 快照文件，避免被项目 .env.local 覆盖回高风险路径。
    AI_WEEKLY_ENV_FILE: args.launchdEnvFilePath,
    AI_WEEKLY_RUNTIME_ROOT: args.runtimeRoot,
    RUNTIME_CONFIG_PATH: pickSafePath(sourceEnv.RUNTIME_CONFIG_PATH, path.join(args.runtimeRoot, "runtime-config", "global.json")),
    STORAGE_DB_PATH: pickSafePath(sourceEnv.STORAGE_DB_PATH, path.join(args.runtimeRoot, "db", "app.sqlite")),
    REVIEW_INSTRUCTION_ROOT: pickSafePath(
      sourceEnv.REVIEW_INSTRUCTION_ROOT,
      path.join(args.runtimeRoot, "review-instructions"),
    ),
    DAEMON_MARKER_ROOT: pickSafePath(sourceEnv.DAEMON_MARKER_ROOT, path.join(args.runtimeRoot, "daemon", "schedule-markers")),
    FEISHU_NOTIFICATION_ROOT: pickSafePath(
      sourceEnv.FEISHU_NOTIFICATION_ROOT,
      path.join(args.runtimeRoot, "notifications", "feishu"),
    ),
    NOTIFICATION_ROOT: pickSafePath(sourceEnv.NOTIFICATION_ROOT, path.join(args.runtimeRoot, "notifications", "feishu")),
    WATCH_LOCK_FILE: pickSafePath(sourceEnv.WATCH_LOCK_FILE, path.join(args.runtimeRoot, "watchdog", "weekly.lock")),
    WATCH_SUMMARY_ROOT: pickSafePath(sourceEnv.WATCH_SUMMARY_ROOT, path.join(args.runtimeRoot, "watchdog")),
    // 运行态迁出仓库后，自动 Git 同步只保留真正需要发布到仓库的 report 目录。
    GIT_SYNC_INCLUDE_PATHS: "outputs/review,outputs/published",
  } satisfies Record<string, string>;
  return managed;
}

function pickSafePath(sourceValue: string | undefined, fallbackValue: string): string {
  // launchd 下的相对路径通常仍会解析到项目仓库内，因此只保留“绝对且低风险”的用户自定义路径。
  if (sourceValue && path.isAbsolute(sourceValue) && !isLikelyTccProtectedPath(sourceValue)) {
    return sourceValue;
  }
  return fallbackValue;
}

function mergeLaunchdEnvContent(input: { sourceContent: string; managedEnv: Record<string, string> }) {
  const managedLines = Object.entries(input.managedEnv).map(([key, value]) => `${key}=${JSON.stringify(value)}`);
  const source = input.sourceContent.trimEnd();
  return `${source}\n\n# --- managed by service-ops: launchd runtime paths ---\n${managedLines.join("\n")}\n`;
}
...
function buildDaemonCommand(args: CliArgs, launchdEnvFilePath: string) {
  ...
  // launchd 直接执行编译后的 JS 入口，绕开 Documents 下 `node_modules/.bin/tsx` 被 TCC 拦截的风险。
  return `cd ${root} && if [ -f ${envFile} ]; then set -a; source ${envFile}; set +a; fi && exec ${nodeBin} dist/cli.js run --daemon`;
}
...
async function buildProjectForLaunchd(args: Pick<CliArgs, "projectRoot">, runner: CommandRunner) {
  const result = await runner({
    command: "/bin/zsh",
    args: ["-lc", `cd ${shellSingleQuote(args.projectRoot)} && pnpm build`],
  });
  if (result.exitCode !== 0) {
    throw new Error(`launchd_build_failed:${result.stderr || result.stdout}`);
  }
}
...
export const __test__ = {
  buildManagedLaunchdEnv,
  parseArgs,
  parseEnvFile,
  isEnvSourceSameAsLaunchdTarget,
  mergeLaunchdEnvContent,
